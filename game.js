...
function generateWorld(){
    for(let y=0;y<WS;y++){
        ...
        for(let x=0;x<WS;x++){
            const cx=x-WS/2,cy=y-WS/2; const dist=Math.sqrt(cx*cx+cy*cy)/(WS*0.46);
...
function breakCoop(reason) {
    ...
    generateWorld(); spawnWilds(); spawnMega(); G.cam.x = G.player.x - canvas.width/2; G.cam.y = G.player.y - canvas.height/2;
...
function exitBattle(){
    ...
    G.lastDamageTick = G.tick; G.encCd=200;G.btns=[];
}
...
function update(){
    ...
    G.particles=G.particles.filter(p=>p.life>0); G.particles.forEach(p=>{p.x+=p.vx;p.y+=p.vy;p.vy+=0.08;p.life--;});
}
...
function updateWorld(){
    ...
    G.cam.x=Math.max(0,Math.min(WS*TS-canvas.width,G.cam.x)); G.cam.y=Math.max(0,Math.min(WS*TS-canvas.height,G.cam.y));
    ...
    if(G.encCd<=0){
        for(let i=0;i<G.wilds.length;i++){
            const w=G.wilds[i]; const dist=Math.hypot(p.x-w.x,p.y-w.y); const trigger=38+DINOS[w.key].sz;
            if(dist<trigger){
                const wasBoss=w.isBoss; G.wilds.splice(i,1); startBattle(w.key,wasBoss);
    ...
    for(let id in G.otherPlayers) {
        ...
        let op = G.otherPlayers[id]; if(!op) continue;
        if(Math.hypot(p.x - op.x, p.y - op.y) < 60) { closeId = id; break; }
    }
...
function startGame(isNew = false) {
    if (!isNew && window.activeSave) {
        ...
        G.coop = { partnerId: null, partnerName: '', reqTo: null, reqFrom: null, reqFromName: '' }; G.soloStash = null;
    } else if (isNew) {
