...
function getCol(c, size) {
    ...
    if(c.startsWith('grad:')) {
        const pts = c.split(':'); const g = ctx.createLinearGradient(-size, -size, size, size);
        g.addColorStop(0, pts[1]); g.addColorStop(1, pts[2]); return g;
    } return c;
}

// ── DINO DRAW ──
function drawDino(key,cx,cy,face,af,sc,alpha,oc){
    const d=DINOS[key];if(!d)return; ctx.save(); ctx.globalAlpha=alpha!=null?alpha:1;
    ctx.translate(cx,cy);ctx.scale(face,1); const s=(sc||1)*(d.sz/20); const bob=Math.sin(af*0.18)*2.5;
    const cBody = oc && oc.body ? getCol(oc.body, 20*s) : d.col; const cLegs = oc && oc.legs ? getCol(oc.legs, 20*s) : d.acc;
    const cHead = oc && oc.head ? getCol(oc.head, 15*s) : d.col; const cNeck = oc && oc.neck ? getCol(oc.neck, 15*s) : d.col;
    const cTail = oc && oc.tail ? getCol(oc.tail, 20*s) : d.col;
    ctx.fillStyle='rgba(0,0,0,0.18)';ctx.beginPath(); ctx.ellipse(0,d.sz*s*0.55+2,d.sz*s*0.65,5,0,0,Math.PI*2);ctx.fill();

    if(key==='megalodon' || key==='mosasaurus'){
        const isMosa = key==='mosasaurus'; ctx.fillStyle=cBody;ctx.beginPath();ctx.ellipse(0,bob,(isMosa?40:46)*s,(isMosa?15:22)*s,0.2,0,Math.PI*2);ctx.fill();
        ctx.fillStyle=cLegs;ctx.beginPath(); ctx.moveTo(5*s,-15*s+bob);ctx.lineTo(-10*s,5*s+bob);ctx.lineTo(20*s,5*s+bob);ctx.fill();
        ctx.fillStyle=cBody;ctx.beginPath(); ctx.moveTo(-10*s,5*s+bob);ctx.lineTo(-35*s,(isMosa?10:20)*s+bob);ctx.lineTo(-5*s,18*s+bob);ctx.fill();
        if(!isMosa){ ctx.beginPath();ctx.moveTo(-10*s,5*s+bob);ctx.lineTo(-35*s,-5*s+bob);ctx.lineTo(-5*s,3*s+bob);ctx.fill(); }
        else{ ctx.beginPath();ctx.moveTo(-35*s,10*s+bob);ctx.lineTo(-50*s,15*s+bob);ctx.lineTo(-45*s,0*s+bob);ctx.fill(); ctx.fillStyle=cLegs; ctx.beginPath();ctx.ellipse(15*s,15*s+bob,8*s,4*s,0.5,0,Math.PI*2);ctx.fill(); ctx.beginPath();ctx.ellipse(-10*s,12*s+bob,7*s,3*s,0.5,0,Math.PI*2);ctx.fill(); }
        ctx.fillStyle='#ffee00';ctx.beginPath();ctx.arc(30*s,-5*s+bob,4*s,0,Math.PI*2);ctx.fill(); ctx.fillStyle='#000';ctx.beginPath();ctx.arc(31*s,-5*s+bob,2*s,0,Math.PI*2);ctx.fill();
        ctx.fillStyle='#fff'; for(let i=-3;i<=3;i++){ctx.beginPath();ctx.moveTo(i*8*s,12*s+bob);ctx.lineTo(i*8*s-3.5*s,20*s+bob);ctx.lineTo(i*8*s+3.5*s,20*s+bob);ctx.fill();}
        if(d.rarity==='Boss'){ ctx.shadowColor='#ff3333';ctx.shadowBlur=30; ctx.strokeStyle='rgba(255,50,50,0.4)';ctx.lineWidth=3; ctx.beginPath();ctx.ellipse(0,bob,49*s,25*s,0.2,0,Math.PI*2);ctx.stroke();ctx.shadowBlur=0; }
    } else if(key==='pterodactyl' || key==='quetzalcoatlus'){
        const isQuetz = key==='quetzalcoatlus'; const flap=Math.sin(af*0.28)*14; ctx.fillStyle=cBody;ctx.beginPath();
        ctx.moveTo(-32*s,flap*s+bob);ctx.quadraticCurveTo(-42*s,12*s+bob,-16*s,16*s+bob); ctx.quadraticCurveTo(0,6*s+bob,16*s,16*s+bob); ctx.quadraticCurveTo(42*s,12*s+bob,32*s,flap*s+bob); ctx.quadraticCurveTo(0,-20*s+bob,-32*s,flap*s+bob);ctx.fill();
        ctx.fillStyle=cLegs;ctx.beginPath();ctx.ellipse(20*s,-6*s+bob,11*s,7*s,-0.3,0,Math.PI*2);ctx.fill(); ctx.fillStyle='#d4a030';ctx.beginPath(); ctx.moveTo(29*s,-6*s+bob);ctx.lineTo((isQuetz?50:42)*s,-3*s+bob);ctx.lineTo(29*s,0+bob);ctx.fill();
        ctx.fillStyle='#ff0';ctx.beginPath();ctx.arc(23*s,-8*s+bob,2.5*s,0,Math.PI*2);ctx.fill(); ctx.fillStyle='#111';ctx.beginPath();ctx.arc(23*s,-8*s+bob,1.2*s,0,Math.PI*2);ctx.fill();
    } else {
        ...
        if(key==='brachiosaurus' || key==='diplodocus'){
            ctx.fillStyle=cNeck;ctx.beginPath(); ctx.moveTo(3*s,-3*s+bob);ctx.quadraticCurveTo(20*s,-20*s+bob,15*s,-38*s+bob); ctx.lineTo(22*s,-38*s+bob);ctx.quadraticCurveTo(27*s,-20*s+bob,10*s,-3*s+bob);ctx.fill();
            ctx.fillStyle=cHead;ctx.beginPath();ctx.ellipse(18*s,-41*s+bob,8*s,5*s,-0.4,0,Math.PI*2);ctx.fill(); ctx.fillStyle='#ffee00';ctx.beginPath();ctx.arc(23*s,-44*s+bob,2.5*s,0,Math.PI*2);ctx.fill(); ctx.fillStyle='#111';ctx.beginPath();ctx.arc(23*s,-44*s+bob,1.2*s,0,Math.PI*2);ctx.fill();
        } else {
            ctx.fillStyle=cNeck;ctx.beginPath();ctx.ellipse(18*s,-8*s+bob,10*s,7*s,-0.35,0,Math.PI*2);ctx.fill(); ctx.fillStyle=cHead;ctx.beginPath(); ctx.moveTo(26*s,-5*s+bob);ctx.lineTo(32*s,-9*s+bob);ctx.lineTo(32*s,-4*s+bob);ctx.fill();
            ctx.fillStyle=(key==='indominus')?'#ff0000':'#ffee00';ctx.beginPath();ctx.arc(22*s,-11*s+bob,2.5*s,0,Math.PI*2);ctx.fill(); ctx.fillStyle='#111';ctx.beginPath();ctx.arc(22.5*s,-11*s+bob,1.2*s,0,Math.PI*2);ctx.fill();
        }
        ctx.fillStyle=cTail;ctx.beginPath(); ctx.moveTo(-18*s,0+bob);ctx.quadraticCurveTo(-30*s,-6*s+bob,-42*s,4*s+bob); ctx.quadraticCurveTo(-30*s,12*s+bob,-18*s,9*s+bob);ctx.fill();
        ctx.fillStyle=cLegs; const la=Math.sin(af*0.22)*6; ctx.fillRect(-5*s,8*s+bob,5.5*s,(13+la)*s); ctx.fillRect(5*s,8*s+bob,5.5*s,(13-la)*s);
        if(d.sz>=28){ ctx.fillRect(-14*s,8*s+bob,5.5*s,(13-la)*s); ctx.fillRect(-24*s,8*s+bob,5.5*s,(13+la)*s); }
        if(['trex','carnotaurus','indoraptor','dilophosaurus','baryonyx','indominus','giganotosaurus'].includes(key)){ ctx.fillStyle=cLegs; ctx.fillRect(10*s,-2*s+bob,3*s,8*s);ctx.fillRect(12*s,5*s+bob,4*s,3*s); }
        if(key==='stegosaurus'){ ctx.fillStyle='#d09040'; for(let i=-2;i<=2;i++){ctx.beginPath();ctx.moveTo(i*7*s,-5*s+bob);ctx.lineTo(i*7*s-3*s,-20*s+bob);ctx.lineTo(i*7*s+3*s,-20*s+bob);ctx.fill();} ctx.fillStyle='#aa3030';ctx.fillRect(-28*s,5*s+bob,4*s,4*s);ctx.fillRect(-32*s,3*s+bob,4*s,4*s); }
        ...
        if(key==='parasaurolophus'){ ctx.fillStyle='#cc6030';ctx.beginPath(); ctx.moveTo(15*s,-10*s+bob);ctx.quadraticCurveTo(5*s,-30*s+bob,-10*s,-20*s+bob); ctx.lineTo(-8*s,-16*s+bob);ctx.quadraticCurveTo(7*s,-22*s+bob,16*s,-8*s+bob);ctx.fill(); }
        if(key==='indoraptor' || key==='indominus'){
            if(key==='indoraptor'){ ctx.fillStyle='rgba(180,0,255,0.3)';ctx.beginPath();ctx.ellipse(0,0+bob,20*s,12*s,0,0,Math.PI*2);ctx.fill(); ctx.fillStyle='#ffff00'; for(let i=0;i<3;i++){ctx.fillRect((-6+i*6)*s,-6*s+bob,2.5*s,12*s);} }
            else{ ctx.fillStyle=cLegs; for(let i=-2;i<=3;i++){ ctx.beginPath();ctx.moveTo(i*8*s,-10*s+bob); ctx.lineTo(i*8*s-4*s,-22*s+bob); ctx.lineTo(i*8*s+4*s,-22*s+bob);ctx.fill(); } }
        }
        ...
        if(d.rarity==='Legendary'){ ctx.shadowColor=RARITY_COLOR.Legendary;ctx.shadowBlur=25; ctx.strokeStyle='rgba(255,170,0,0.5)';ctx.lineWidth=2; ctx.beginPath(); ctx.ellipse(0,0+bob,23*s,15*s,0,0,Math.PI*2);ctx.stroke();ctx.shadowBlur=0; }
        if(d.rarity==='Epic'){ ctx.shadowColor=RARITY_COLOR.Epic;ctx.shadowBlur=18; ctx.strokeStyle='rgba(200,68,255,0.4)';ctx.lineWidth=1.5; ctx.beginPath(); ctx.ellipse(0,0+bob,23*s,15*s,0,0,Math.PI*2);ctx.stroke();ctx.shadowBlur=0; }
        if(d.rarity==='Boss' && key==='indominus'){ ctx.shadowColor='#ff3333';ctx.shadowBlur=30; ctx.strokeStyle='rgba(255,50,50,0.5)';ctx.lineWidth=3; ctx.beginPath(); ctx.ellipse(0,0+bob,25*s,16*s,0,0,Math.PI*2);ctx.stroke();ctx.shadowBlur=0; }
    } ctx.restore();
}
...
function drawHat(type, cx, cy, sc){
    ...
    if (!type || type === 'bucket') {
        ctx.save();ctx.translate(-11*s,-18*s);ctx.rotate(-0.35); const hg1=ctx.createLinearGradient(0,-20*s,0,0);hg1.addColorStop(0,'#00ffee');hg1.addColorStop(1,'#009999'); ctx.fillStyle=hg1;ctx.beginPath();ctx.moveTo(0,0);ctx.lineTo(-4*s,-22*s);ctx.lineTo(4*s,-22*s);ctx.closePath();ctx.fill(); ctx.strokeStyle='rgba(0,255,220,0.5)';ctx.lineWidth=1*s;ctx.stroke();ctx.restore();
        ctx.save();ctx.translate(11*s,-18*s);ctx.rotate(0.35); const hg2=ctx.createLinearGradient(0,-20*s,0,0);hg2.addColorStop(0,'#00ffee');hg2.addColorStop(1,'#009999'); ctx.fillStyle=hg2;ctx.beginPath();ctx.moveTo(0,0);ctx.lineTo(-4*s,-22*s);ctx.lineTo(4*s,-22*s);ctx.closePath();ctx.fill(); ctx.strokeStyle='rgba(0,255,220,0.5)';ctx.lineWidth=1*s;ctx.stroke();ctx.restore();
        ctx.fillStyle='#2a2e36';ctx.beginPath(); ctx.moveTo(-18*s,-18*s);ctx.lineTo(18*s,-18*s);ctx.lineTo(14*s,4*s);ctx.lineTo(-14*s,4*s);ctx.closePath();ctx.fill(); const rimG=ctx.createLinearGradient(0,-22*s,0,-16*s);rimG.addColorStop(0,'#4a5060');rimG.addColorStop(1,'#1e2228');
        ctx.fillStyle=rimG;ctx.beginPath();ctx.ellipse(0,-18*s,20*s,4.5*s,0,0,Math.PI*2);ctx.fill(); ctx.strokeStyle='#555d6a';ctx.lineWidth=1.2*s;ctx.stroke(); ctx.fillStyle='#1e2228';ctx.beginPath();ctx.ellipse(0,4*s,14.5*s,3.5*s,0,0,Math.PI*2);ctx.fill(); ctx.strokeStyle='#3a4050';ctx.lineWidth=1*s;ctx.stroke();
        ctx.strokeStyle='rgba(80,90,110,0.6)';ctx.lineWidth=1*s; for(let i=-1;i<=1;i++){ctx.beginPath();ctx.moveTo(i*7*s,-14*s);ctx.lineTo(i*6*s,2*s);ctx.stroke();} ctx.fillStyle='#c8a820';ctx.font=`bold ${9*s}px Courier New`; ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText('AA',-1*s,-8*s);
    } else if (type === 'bacon') {
        ctx.fillStyle = '#cc6600'; ctx.beginPath(); ctx.ellipse(0, -8*s, 16*s, 10*s, 0, 0, Math.PI*2); ctx.fill();
        ctx.fillStyle = '#e67300'; const spikes = [[-10,-20,-18,-2],[-2,-24,-8,4],[6,-22,12,0],[12,-18,18,6]]; spikes.forEach(sp => { ctx.beginPath(); ctx.moveTo(sp[0]*s, sp[1]*s); ctx.quadraticCurveTo(sp[2]*s, sp[3]*s, (sp[2]-4)*s, (sp[3]+8)*s); ctx.lineTo(0, -5*s); ctx.fill(); });
        ctx.fillStyle = '#994d00'; const spikes2 = [[-14,-14,-22,5],[-6,-26,-14,-5],[2,-26,6,-5],[16,-12,22,8]]; spikes2.forEach(sp => { ctx.beginPath(); ctx.moveTo(sp[0]*s, sp[1]*s); ctx.quadraticCurveTo(sp[2]*s, sp[3]*s, (sp[2]+4)*s, (sp[3]+6)*s); ctx.lineTo(0, -5*s); ctx.fill(); });
    } else if (type === 'dino_hood') {
        const hg = ctx.createLinearGradient(0, -20*s, 0, 5*s); hg.addColorStop(0, '#5cd65c'); hg.addColorStop(1, '#2eb82e'); ctx.fillStyle = '#145214'; ctx.beginPath(); ctx.moveTo(0, -18*s); ctx.lineTo(-6*s, -30*s); ctx.lineTo(8*s, -18*s); ctx.fill();
        ctx.beginPath(); ctx.moveTo(-8*s, -10*s); ctx.lineTo(-18*s, -20*s); ctx.lineTo(-12*s, -5*s); ctx.fill(); ctx.fillStyle = hg; ctx.beginPath(); ctx.moveTo(-16*s, 8*s); ctx.quadraticCurveTo(-20*s, -18*s, 0, -22*s);
        ctx.quadraticCurveTo(20*s, -18*s, 16*s, 8*s); ctx.quadraticCurveTo(0, 14*s, -16*s, 8*s); ctx.fill(); ctx.fillStyle = '#ffffe6'; for(let i = -2; i <= 2; i++) { ctx.beginPath(); ctx.moveTo((i*5 - 2)*s, 9*s); ctx.lineTo((i*5)*s, 15*s); ctx.lineTo((i*5 + 2)*s, 9*s); ctx.fill(); }
        ...
        const hg = ctx.createLinearGradient(0, -22*s, 0, 0); hg.addColorStop(0, '#ffe600'); hg.addColorStop(1, '#e6b800'); ctx.fillStyle = hg; ctx.beginPath(); ctx.moveTo(0,0); ctx.lineTo(-12*s, -20*s); ctx.lineTo(12*s, -20*s); ctx.closePath(); ctx.fill();
        ctx.fillStyle = '#cca300'; ctx.beginPath(); ctx.ellipse(0, 4*s, 18*s, 4*s, 0, 0, Math.PI*2); ctx.fill(); ctx.fillStyle = hg; ctx.beginPath(); ctx.ellipse(0, 4*s, 16*s, 3*s, 0, 0, Math.PI*2); ctx.fill();
        ctx.fillStyle = '#ffdb4d'; ctx.beginPath(); ctx.ellipse(0, -20*s, 12*s, 3*s, 0, 0, Math.PI*2); ctx.fill(); ctx.fillStyle = '#1a1a1a'; ctx.beginPath(); ctx.ellipse(-4*s, -10*s, 1.2*s, 2*s, 0, 0, Math.PI*2); ctx.fill(); ctx.beginPath(); ctx.ellipse(4*s, -10*s, 1.2*s, 2*s, 0, 0, Math.PI*2); ctx.fill();
...
function spawnParticles(x,y,col,n){ for(let i=0;i<n;i++){ const a=Math.random()*Math.PI*2,sp=1.5+Math.random()*3; G.particles.push({x,y,vx:Math.cos(a)*sp,vy:Math.sin(a)*sp,col,life:40,maxLife:40}); } }
function updateDrawParticles(){ G.particles=G.particles.filter(p=>p.life>0); for(const p of G.particles){ p.x+=p.vx;p.y+=p.vy;p.vy+=0.1;p.life--; ctx.globalAlpha=p.life/p.maxLife; ctx.fillStyle=p.col;ctx.beginPath();ctx.arc(p.x,p.y,2.5,0,Math.PI*2);ctx.fill(); } ctx.globalAlpha=1; }

// ── WORLD DRAW ──
function drawWorld(){
    const sx0=Math.floor(G.cam.x/TS)-1, sy0=Math.floor(G.cam.y/TS)-1; const sx1=sx0+Math.ceil(canvas.width/TS)+2, sy1=sy0+Math.ceil(canvas.height/TS)+2;
    for(let ty=Math.max(0,sy0);ty<Math.min(WS,sy1);ty++){
        for(let tx=Math.max(0,sx0);tx<Math.min(WS,sx1);tx++){
            const px=tx*TS-G.cam.x, py=ty*TS-G.cam.y; const t=worldMap[ty][tx]; ctx.fillStyle=tileClr[ty][tx];ctx.fillRect(px,py,TS+1,TS+1);
            ...
            if(t===4){
                ...
                ctx.fillStyle = '#2a0500'; ctx.fillRect(px, py, TS, TS); ctx.fillStyle = 'rgba(200,40,0,' + (pulse*0.9).toFixed(2) + ')'; ctx.fillRect(px+2, py+2, TS-4, TS-4);
...
function drawHazards() {
    for(const h of G.hazards){
        const sx = h.x - G.cam.x; const sy = h.y - G.cam.y; if(sx < -100 || sx > canvas.width + 100 || sy < -100 || sy > canvas.height + 100) continue;
        ...
        ctx.fillStyle = '#4a2010'; ctx.beginPath(); ctx.arc(sx, rockY, 18, 0, Math.PI * 2); ctx.fill(); ctx.fillStyle = '#ff4400'; ctx.beginPath(); ctx.arc(sx, rockY, 12, 0, Math.PI * 2); ctx.fill();
    }
}
function drawWilds(){
    for(const w of G.wilds){
        const sx=w.x-G.cam.x, sy=w.y-G.cam.y; if(sx<-130||sx>canvas.width+130||sy<-130||sy>canvas.height+130) continue;
        if(w.isBoss){
            const pulse=Math.sin(G.tick*0.05)*0.15+0.15; ctx.fillStyle=`rgba(255,30,30,${pulse})`;ctx.beginPath();ctx.arc(sx,sy,90,0,Math.PI*2);ctx.fill();
        ...
        drawDino(w.key,sx,sy,w.face,w.anim,1,1); const dn=DINOS[w.key];
        ctx.fillStyle='rgba(0,0,0,0.65)';ctx.fillRect(sx-32,sy-dn.sz-25,64,14); ctx.fillStyle=RARITY_COLOR[dn.rarity];ctx.font='bold 9px Courier New'; ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(dn.name,sx,sy-dn.sz-18);
    }
}
function drawPlayer(){
    const p=G.player; const sx=p.x-G.cam.x, sy=p.y-G.cam.y; const asz=10, ay=sy-DINOS[p.dk].sz-32+Math.sin(G.tick*0.08)*4;
    ...
    drawDino(p.dk,sx,sy,p.face,p.anim,1.25,1, p.oc); const headOff=DINOS[p.dk].sz*1.25*0.55; const bob=Math.sin(p.anim*0.18)*2.5; drawHat(G.player.hat || 'bucket',sx,sy-headOff+bob-2,1.1);
    const pct=G.playerHp/pMaxHp(); const col=pct>0.5?'#44cc44':pct>0.25?'#cccc44':'#cc4444'; hpBar(sx-28,sy-DINOS[p.dk].sz-18,56,8,G.playerHp,pMaxHp(),col);
...
function drawHUD(){
    ...
    if (G.coop.partnerId) {
        btn(16, 56, 110, 20, 'Leave Co-op', '#cc2244', '#fff', () => { if(confirm("Warning: Leaving Co-op will break the party and reset your progress back to Solo Level 1. Continue?")) { sendCoop({ type: 'coop_break', target: G.coop.partnerId }); breakCoop("You left the Co-op party."); } }, '💔');
    }
    ...
    if (G.coop.reqFrom) {
        ...
        btn(rx + 150, ry + 45, 110, 26, 'Accept', '#22ccaa', '#fff', () => {
            ...
            else { sendCoop({ type: 'coop_reply', target: G.coop.reqFrom, sender: (G.isHost ? 'host' : G.peer.id), accept: false, name: G.username || 'Player' }); } G.coop.reqFrom = null;
        }, '🤝');
        ...
        if (op) {
            ...
            if (!G.coop.partnerId && !op.coopPartner && !G.coop.reqTo) {
                btn(W/2 - 160, H - 150, 140, 36, 'Add Friend', '#22ccaa', '#fff', () => { if(confirm("WARNING: Sending a Co-op request will permanently reset your Solo Progress if accepted. Continue?")) { G.coop.reqTo = G.pvp.closeId; sendCoop({ type: 'coop_request', target: G.pvp.closeId, sender: (G.isHost ? 'host' : G.peer.id), name: G.username || 'Player' }); addChatMessage('System', 'Co-op Request Sent!'); } }, '🤝');
            }
        }
    }
...
function drawBattle(){
    ...
    if (b.isCoop) {
        ...
        // P1
        drawDino(pDk, W*0.25+pshake, H*0.4, 1, G.tick, 1.4, b.php>0?1:0.3, G.player.oc); drawHat(G.player.hat || 'bucket', W*0.25+pshake, H*0.4-DINOS[pDk].sz*1.4*0.55, 1.3);
        ...
        // P2
        drawDino(b.cpDk, W*0.25+cshake, H*0.65, 1, G.tick, 1.4, b.cpHp>0?1:0.3, b.cpOc); drawHat(b.cpHat || 'bucket', W*0.25+cshake, H*0.65-DINOS[b.cpDk].sz*1.4*0.55, 1.3);
        ...
        const pshake=b.pshake>0?(Math.random()-0.5)*8:0; drawDino(pDk,W*0.28+pshake,H*0.52,1,G.tick,1.65,1, G.player.oc); drawHat(G.player.hat || 'bucket',W*0.28+pshake,H*0.52-DINOS[pDk].sz*1.65*0.55,1.5);
    ...
    b.log.slice(0,4).forEach((line,i)=>{ ctx.fillStyle=i===0?'#ffffff':`rgba(255,255,255,${0.55-i*0.12})`; ctx.font=`${i===0?'bold ':''}12px Courier New`; ctx.textAlign='left';ctx.textBaseline='top'; ctx.fillText(line,W*0.05+12,logY+10+i*23); });
    for(const dn of b.dnums){ ctx.globalAlpha=dn.life/60; ctx.fillStyle=dn.col;ctx.font=`bold ${Math.max(10,26-dn.life*0.1)}px Courier New`; ctx.textAlign='center';ctx.textBaseline='middle'; ctx.fillText(`-${dn.val}`,dn.x,dn.y-(60-dn.life)*0.9); }
    ctx.globalAlpha=1;

    if(b.res === 'level2'){
        rr(W/2-165,H/2-70,330,140,12,'rgba(40,0,40,0.92)','#cc44ff',3); ctx.fillStyle='#cc44ff';ctx.shadowColor='#cc44ff';ctx.shadowBlur=15; ctx.font='bold 26px Courier New';ctx.textAlign='center';ctx.textBaseline='middle'; ctx.fillText('🏆 MEGALODON DEFEATED!',W/2,H/2-35);ctx.shadowBlur=0; ctx.fillStyle='#ddd';ctx.font='13px Courier New'; ctx.fillText('The earth shakes... Map 2 Unlocked!',W/2,H/2);
        btn(W/2-85,H/2+25,170,40,'Enter Level 2','#8822cc','#fff',()=>{
            G.level = 2; G.discovered = {utahraptor: true}; G.player.dk = 'utahraptor'; generateWorld(); spawnWilds(); spawnMega(); G.player.x = WS/2*TS; G.player.y = WS/2*TS; G.volcanoTimer = 10800; G.volcanoActive = 0; G.hazards =[]; G.cam.x = G.player.x - canvas.width/2; G.cam.y = G.player.y - canvas.height/2; exitBattle(); G.playerHp = pMaxHp(); G.playerShield = 0; if (G.coop.partnerId) sendCoop({ type: 'coop_sync', level: 2 });
        },'🌋');
    } else if(!b.res){
        const bx=W*0.65, by= b.isCoop ? H*0.83 : H*0.73;
        if (b.isCoop && b.turn === 'partner') { ctx.fillStyle='#aaddff'; ctx.font='bold 14px Courier New'; ctx.textAlign='center'; ctx.textBaseline='middle'; ctx.fillText(`⏳ Waiting for ${b.cpName}...`, bx + 65, by + 20); } 
        else if (b.isCoop && b.turn === 'enemy') { ctx.fillStyle='#ff8844'; ctx.font='bold 14px Courier New'; ctx.textAlign='center'; ctx.textBaseline='middle'; ctx.fillText(`▶ Enemy Turn...`, bx + 65, by + 20); } 
        else {
        ...
        const win=b.res==='win'; rr(W/2-165,H/2-70,330,140,12,win?'rgba(0,80,0,0.92)':'rgba(80,0,0,0.92)',win?'#44ff44':'#ff4444',3); ctx.fillStyle=win?'#44ff44':'#ff4444';ctx.shadowColor=win?'#44ff44':'#ff4444';ctx.shadowBlur=15; ctx.font='bold 30px Courier New';ctx.textAlign='center';ctx.textBaseline='middle'; ctx.fillText(win?'🏆 VICTORY!':'💀 DEFEATED!',W/2,H/2-35);ctx.shadowBlur=0; ctx.fillStyle='#ddd';ctx.font='13px Courier New'; ctx.fillText(b.log[0]||'',W/2,H/2); btn(W/2-70,H/2+25,140,40,'Continue','#224488','#fff',exitBattle,'▶');
    }
}

// ── INDEX, SHOP, CUSTOMIZE, INTRO ──
function drawIndex(){
    G.btns=[]; const W=canvas.width,H=canvas.height; ctx.fillStyle='#070d1a';ctx.fillRect(0,0,W,H);
    ...
    ctx.fillStyle='rgba(10,25,60,0.95)';ctx.fillRect(0,0,W,52); ctx.strokeStyle='rgba(100,150,255,0.3)';ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(0,52);ctx.lineTo(W,52);ctx.stroke(); ctx.fillStyle='#6699ff';ctx.shadowColor='#4477ff';ctx.shadowBlur=15; ctx.font='bold 24px Courier New';ctx.textAlign='center';ctx.textBaseline='middle'; ctx.fillText(G.level===1?'📘 DINO INDEX (MAP 1)':'📘 DINO INDEX (MAP 2)',W/2,26);ctx.shadowBlur=0;
    const ak=Object.keys(DINOS).filter(k=>DINOS[k].lvl === G.level); ctx.fillStyle='#888';ctx.font='12px Courier New';ctx.textAlign='left';ctx.textBaseline='middle'; ctx.fillText(`${ak.filter(k=>G.discovered[k]).length}/${ak.length} Discovered`,20,26);
    const cw=Math.min(145,Math.floor((W-30)/Math.max(1,Math.floor((W-30)/145)))); const cols=Math.max(1,Math.floor((W-30)/cw)); const rows=Math.max(1,Math.floor((H-90)/130)); const per=cols*rows; const pg=G.idxPage; const si=pg*per, ei=Math.min(si+per,ak.length);
    for(let i=si;i<ei;i++){
        const key=ak[i], dn=DINOS[key]; const col=(i-si)%cols, row=Math.floor((i-si)/cols); const cx=15+col*cw+cw/2, cy=68+row*130+62; const found=G.discovered[key], isCur=G.player.dk===key;
        ...
        if(found){ ctx.save();ctx.beginPath();ctx.rect(cx-cw/2+6,cy-55,cw-12,80);ctx.clip(); const idxOc=isCur?G.player.oc:null; drawDino(key,cx,cy-18,1,G.tick*0.5,0.65,1,idxOc);ctx.restore(); } 
        ...
        if(found){
            ...
            if(!isCur) btn(cx-32,cy+61,64,22,'Equip','#1a3a88','#fff',()=>{ if(G.swapCd>0) return; const oldHpPct = Math.min(1, G.playerHp / pMaxHp()); G.player.dk=key; G.playerHp = Math.max(1, Math.floor(pMaxHp() * oldHpPct)); G.playerShield = Math.min(G.playerShield, Math.floor(pMaxHp() * 0.4)); G.swapCd = 120; const ttx = Math.floor(G.player.x/TS); const tty = Math.floor(G.player.y/TS); const canSwim =['spinosaurus','pterodactyl','megalodon','mosasaurus'].includes(key); if(worldMap[tty] && worldMap[tty][ttx] === 2 && !canSwim){ G.player.x = WS/2*TS; G.player.y = WS/2*TS; } });
...
function drawShop(){
    G.btns=[]; const W=canvas.width,H=canvas.height; ctx.fillStyle='#0a0602';ctx.fillRect(0,0,W,H);
    ...
    upgs.forEach((upg,i)=>{
        ...
        ctx.font='36px sans-serif';ctx.textAlign='center';ctx.textBaseline='top';ctx.fillText(upg.icon,x+cw2/2,y+14); ctx.fillStyle='#fff';ctx.font='bold 15px Courier New';ctx.textBaseline='top';ctx.fillText(upg.label,x+cw2/2,y+62);
        ctx.fillStyle=maxed?'#ffaa00':'#ffd700';ctx.font='bold 13px Courier New'; ctx.fillText(maxed?`Lv ${lv} ✓`:`Lv ${lv}`,x+cw2/2,y+83); ctx.fillStyle='#aaa';ctx.font='11px Courier New';ctx.fillText(upg.desc,x+cw2/2,y+104);
        ...
        btn(x+16,y+154,cw2-32,42,maxed?'MAX UPGRADE':'UPGRADE', maxed?'#553300':can?upg.col:'#2a2020', maxed?'#ffaa00':can?'#fff':'#665', ()=>{ if(!maxed&&G.wheat>=cost){ G.wheat-=cost;G.player.upg[upg.k]++; if(upg.k==='hp') G.playerHp += Math.floor(28 * R_MULT[pDino().rarity]); spawnParticles(x+cw2/2,y+175,'#55ccff',10); if (G.coop.partnerId) sendCoop({ type: 'coop_wheat', target: G.coop.partnerId, wheat: G.wheat }); } });
    ...
    rr(hx,hy,hhw,80,10,'rgba(0,30,50,0.95)',scan?'#44aaff':'#1a2a3a',2); ctx.font='24px sans-serif';ctx.fillText('🛡️',hx+hhw/2,hy+10); ctx.fillStyle='#aaddff';ctx.font='bold 14px Courier New';ctx.fillText('+15% Shield',hx+hhw/2,hy+42); ctx.fillText(`Cost: 🪣 ${scost}`,hx+hhw/2,hy+60);
    btn(hx,hy,hhw,80,'',scan?'rgba(0,0,0,0.01)':'rgba(0,0,0,0.01)','rgba(0,0,0,0)',()=>{ const maxS = Math.floor(pMaxHp()*0.4); const addS = Math.floor(pMaxHp()*0.15); if(G.wheat>=scost && G.playerShield < maxS){G.wheat-=scost;G.playerShield=Math.min(maxS, G.playerShield+addS);spawnParticles(W/2,H/2,'#44aaff',14); if (G.coop.partnerId) sendCoop({ type: 'coop_wheat', target: G.coop.partnerId, wheat: G.wheat });} });

    const hcost = 5; const hcan = G.wheat>=hcost && G.playerHp < pMaxHp();
    rr(hx+hhw+10,hy,hhw,80,10,'rgba(0,30,0,0.95)',hcan?'#44aa44':'#1a2a1a',2); ctx.font='24px sans-serif';ctx.fillText('💊',hx+hhw+10+hhw/2,hy+10); ctx.fillStyle='#aaffaa';ctx.font='bold 14px Courier New';ctx.fillText('+10% Heal',hx+hhw+10+hhw/2,hy+42); ctx.fillText(`Cost: 🪣 ${hcost}`,hx+hhw+10+hhw/2,hy+60);
    btn(hx+hhw+10,hy,hhw,80,'',hcan?'rgba(0,0,0,0.01)':'rgba(0,0,0,0.01)','rgba(0,0,0,0)',()=>{ if(G.wheat>=hcost && G.playerHp < pMaxHp()){G.wheat-=hcost;G.playerHp=Math.min(pMaxHp(), G.playerHp + Math.floor(pMaxHp()*0.1));spawnParticles(W/2,H/2,'#44ff44',14); if (G.coop.partnerId) sendCoop({ type: 'coop_wheat', target: G.coop.partnerId, wheat: G.wheat });} });
...
function drawCustomize(){
    ...
    const px = W/2; const py = 180; const pDk = G.player.dk; drawDino(pDk, px, py, 1, G.tick, 2.5, 1, G.player.oc);
    ...
    hats.forEach((h, i) => {
        const col = i % 2; const row = Math.floor(i / 2); const x = sx + col * (cw + 20); const y = sy + row * (ch + 15); const isEq = (G.player.hat || 'bucket') === h.id;
        rr(x, y, cw, ch, 10, isEq ? 'rgba(80,20,120,0.9)' : 'rgba(20,10,30,0.9)', isEq ? '#dd88ff' : '#442266', 2); drawHat(h.id, x + cw/2, y + 40, 1.6);
    ...
    const colsY = sy + 2*(ch + 15) + 70; ctx.fillStyle = '#fff'; ctx.font = 'bold 14px Courier New'; ctx.fillText('Select Color:', W/2, colsY - 15);
    CUST_COLS.forEach((col, i) => {
        const rx = W/2 - 165 + (i%6)*55; const ry = colsY + Math.floor(i/6)*50; ctx.fillStyle = getCol(col, 18); ctx.beginPath(); ctx.arc(rx+25, ry+20, 18, 0, Math.PI*2); ctx.fill();
...
function drawIntro(){
    G.btns=[]; const W=canvas.width,H=canvas.height; ctx.fillStyle='#03080f';ctx.fillRect(0,0,W,H);
...
function render(){
    ...
    if(G.state==='world'){
        ctx.save(); if(G.camShake > 0) ctx.translate((Math.random()-0.5)*10, (Math.random()-0.5)*10);
        ...
        for (let id in G.otherPlayers) {
            ...
            if (sx > -100 && sx < canvas.width + 100 && sy > -100 && sy < canvas.height + 100) {
                ...
                const headOff = DINOS[op.dk].sz * 1.25 * 0.55; const bob = Math.sin(op.anim * 0.18) * 2.5; drawHat(op.hat || 'bucket', sx, sy - headOff + bob - 2, 1.1);
                ...
                // NEW OVERWORLD HP BAR (To see partner health!)
                const opHp = op.hp || 1; const opMhp = op.mhp || 1; const hpPct = Math.max(0, opHp/opMhp); const hpC = hpPct>0.5?'#44cc44':hpPct>0.25?'#cccc44':'#cc4444';
                ctx.fillStyle='rgba(0,0,0,0.8)'; ctx.fillRect(sx-20, sy - DINOS[op.dk].sz - 16, 40, 5); ctx.fillStyle=hpC; ctx.fillRect(sx-20, sy - DINOS[op.dk].sz - 16, 40 * hpPct, 5); ctx.strokeStyle='rgba(255,255,255,0.3)'; ctx.lineWidth=1; ctx.strokeRect(sx-20, sy - DINOS[op.dk].sz - 16, 40, 5);
            }
        }
        ...
        if(G.level===2 && G.volcanoActive > 0){
            ...
            ctx.fillStyle = '#ff4444'; ctx.shadowColor = '#000'; ctx.shadowBlur = 4; ctx.font = 'bold 22px Courier New'; ctx.textAlign = 'center'; ctx.textBaseline = 'top'; ctx.fillText(`🌋 VOLCANO ERUPTION! DODGE FALLING ROCKS! ${Math.ceil(G.volcanoActive/60)}s`, canvas.width/2, 60); ctx.shadowBlur = 0;
        }
        drawHUD();
        if(G.joy.on){
