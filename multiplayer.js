...
function filterMessage(msg) {
    ...
    for (let k in leetMap) {
        leeted = leeted.split(k).join(leetMap[k]);
    }
...
function addChatMessage(sender, msg) {
    ...
    msgEl.addEventListener('mousedown', touchStart);
    msgEl.addEventListener('mousemove', touchMove);
    msgEl.addEventListener('mouseup', touchEnd);
    msgEl.addEventListener('mouseleave', touchEnd);
    msgEl.addEventListener('touchstart', touchStart, {passive: true});
    msgEl.addEventListener('touchmove', touchMove, {passive: true});
    msgEl.addEventListener('touchend', touchEnd);
...
function hostGame() {
    ...
    G.peer.on('connection', c => {
        c.on('open', () => {
            ...
            c.on('data', data => {
                if (data.type === 'sync') {
                    G.otherPlayers[c.peer] = data.player; 
                } else if (data.type === 'chat') { 
                    addChatMessage(data.sender, data.msg); 
                    for (let id in G.conns) { 
                        if (id !== c.peer && G.conns[id].open) {
                            G.conns[id].send({ type: 'chat', sender: data.sender, msg: data.msg }); 
                        }
                    } 
    ...
    syncInterval = setInterval(() => {
        try {
            ...
            if (G.isHost) {
                ...
                if (G.conns['host'] && G.conns['host'].open) {
                    G.conns['host'].send({ type: 'sync', player: myData }); 
                }
            }
        } catch(e) {}
    }, 100);
