...
function hpBar(x, y, w, h, cur, max, col, label){
    ...
    gc.addColorStop(0, col);
    gc.addColorStop(1, lighten(col,40));
    ...
    if (label) {
        ctx.fillStyle = '#fff';
        ctx.font = `bold ${Math.max(8, h-2)}px Courier New`;
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
...
function getCol(c, size) {
    ...
    if (c.startsWith('grad:')) {
        ...
        g.addColorStop(0, pts[1]);
        g.addColorStop(1, pts[2]);
        return g;
    }
...
function drawDino(key, cx, cy, face, af, sc, alpha, oc){
    ...
    ctx.beginPath();
    ctx.ellipse(0, d.sz*s*0.55 + 2, d.sz*s*0.65, 5, 0, 0, Math.PI*2);
    ctx.fill();
    ...
    if (key === 'megalodon' || key === 'mosasaurus') {
        ...
        ctx.fillStyle = cBody;
        ctx.beginPath();
        ctx.ellipse(0, bob, (isMosa ? 40 : 46)*s, (isMosa ? 15 : 22)*s, 0.2, 0, Math.PI*2);
        ctx.fill();
        
        ctx.fillStyle = cLegs;
        ctx.beginPath();
        ctx.moveTo(5*s, -15*s+bob);
        ctx.lineTo(-10*s, 5*s+bob);
        ctx.lineTo(20*s, 5*s+bob);
        ctx.fill();
        
        ctx.fillStyle = cBody;
        ctx.beginPath();
        ctx.moveTo(-10*s, 5*s+bob);
        ctx.lineTo(-35*s, (isMosa ? 10 : 20)*s+bob);
        ctx.lineTo(-5*s, 18*s+bob);
        ctx.fill();
        
        if (!isMosa) {
            ctx.beginPath();
            ctx.moveTo(-10*s, 5*s+bob);
            ctx.lineTo(-35*s, -5*s+bob);
            ctx.lineTo(-5*s, 3*s+bob);
            ctx.fill();
        } else {
            ctx.beginPath();
            ctx.moveTo(-35*s, 10*s+bob);
            ctx.lineTo(-50*s, 15*s+bob);
            ctx.lineTo(-45*s, 0*s+bob);
            ctx.fill();
            
            ctx.fillStyle = cLegs;
            ctx.beginPath();
            ctx.ellipse(15*s, 15*s+bob, 8*s, 4*s, 0.5, 0, Math.PI*2);
            ctx.fill();
            
            ctx.beginPath();
            ctx.ellipse(-10*s, 12*s+bob, 7*s, 3*s, 0.5, 0, Math.PI*2);
            ctx.fill();
        }
        
        ctx.fillStyle = '#ffee00';
        ctx.beginPath();
        ctx.arc(30*s, -5*s+bob, 4*s, 0, Math.PI*2);
        ctx.fill();
        
        ctx.fillStyle = '#000';
        ctx.beginPath();
        ctx.arc(31*s, -5*s+bob, 2*s, 0, Math.PI*2);
        ctx.fill();
        
        ctx.fillStyle = '#fff';
        for (let i = -3; i <= 3; i++) {
            ctx.beginPath();
            ctx.moveTo(i*8*s, 12*s+bob);
            ctx.lineTo(i*8*s - 3.5*s, 20*s+bob);
            ctx.lineTo(i*8*s + 3.5*s, 20*s+bob);
            ctx.fill();
        }
        ...
        if (d.rarity === 'Boss') {
            ...
            ctx.lineWidth = 3;
            ctx.beginPath();
            ctx.ellipse(0, bob, 49*s, 25*s, 0.2, 0, Math.PI*2);
            ctx.stroke();
            ctx.shadowBlur = 0;
        }
        ...
        ctx.fillStyle = cBody;
        ctx.beginPath();
        ctx.moveTo(-32*s, flap*s+bob);
        ctx.quadraticCurveTo(-42*s, 12*s+bob, -16*s, 16*s+bob);
        ctx.quadraticCurveTo(0, 6*s+bob, 16*s, 16*s+bob);
        ctx.quadraticCurveTo(42*s, 12*s+bob, 32*s, flap*s+bob);
        ctx.quadraticCurveTo(0, -20*s+bob, -32*s, flap*s+bob);
        ctx.fill();
        
        ctx.fillStyle = cLegs;
        ctx.beginPath();
        ctx.ellipse(20*s, -6*s+bob, 11*s, 7*s, -0.3, 0, Math.PI*2);
        ctx.fill();
        
        ctx.fillStyle = '#d4a030';
        ctx.beginPath();
        ctx.moveTo(29*s, -6*s+bob);
        ctx.lineTo((isQuetz ? 50 : 42)*s, -3*s+bob);
        ctx.lineTo(29*s, 0+bob);
        ctx.fill();
        
        ctx.fillStyle = '#ff0';
        ctx.beginPath();
        ctx.arc(23*s, -8*s+bob, 2.5*s, 0, Math.PI*2);
        ctx.fill();
        
        ctx.fillStyle = '#111';
        ctx.beginPath();
        ctx.arc(23*s, -8*s+bob, 1.2*s, 0, Math.PI*2);
        ctx.fill();
    } else {
        ctx.fillStyle = cBody;
        ctx.beginPath();
        ctx.ellipse(0, 0+bob, 21*s, 13*s, 0, 0, Math.PI*2);
        ctx.fill();
        
        if (key === 'brachiosaurus' || key === 'diplodocus') {
            ctx.fillStyle = cNeck;
            ctx.beginPath();
            ctx.moveTo(3*s, -3*s+bob);
            ctx.quadraticCurveTo(20*s, -20*s+bob, 15*s, -38*s+bob);
            ctx.lineTo(22*s, -38*s+bob);
            ctx.quadraticCurveTo(27*s, -20*s+bob, 10*s, -3*s+bob);
            ctx.fill();
            
            ctx.fillStyle = cHead;
            ctx.beginPath();
            ctx.ellipse(18*s, -41*s+bob, 8*s, 5*s, -0.4, 0, Math.PI*2);
            ctx.fill();
            
            ctx.fillStyle = '#ffee00';
            ctx.beginPath();
            ctx.arc(23*s, -44*s+bob, 2.5*s, 0, Math.PI*2);
            ctx.fill();
            
            ctx.fillStyle = '#111';
            ctx.beginPath();
            ctx.arc(23*s, -44*s+bob, 1.2*s, 0, Math.PI*2);
            ctx.fill();
        } else {
            ctx.fillStyle = cNeck;
            ctx.beginPath();
            ctx.ellipse(18*s, -8*s+bob, 10*s, 7*s, -0.35, 0, Math.PI*2);
            ctx.fill();
            
            ctx.fillStyle = cHead;
            ctx.beginPath();
            ctx.moveTo(26*s, -5*s+bob);
            ctx.lineTo(32*s, -9*s+bob);
            ctx.lineTo(32*s, -4*s+bob);
            ctx.fill();
            
            ctx.fillStyle = (key === 'indominus') ? '#ff0000' : '#ffee00';
            ctx.beginPath();
            ctx.arc(22*s, -11*s+bob, 2.5*s, 0, Math.PI*2);
            ctx.fill();
            
            ctx.fillStyle = '#111';
            ctx.beginPath();
            ctx.arc(22.5*s, -11*s+bob, 1.2*s, 0, Math.PI*2);
            ctx.fill();
        }
        
        ctx.fillStyle = cTail;
        ctx.beginPath();
        ctx.moveTo(-18*s, 0+bob);
        ctx.quadraticCurveTo(-30*s, -6*s+bob, -42*s, 4*s+bob);
        ctx.quadraticCurveTo(-30*s, 12*s+bob, -18*s, 9*s+bob);
        ctx.fill();
        
        ctx.fillStyle = cLegs;
        const la = Math.sin(af * 0.22) * 6;
        ctx.fillRect(-5*s, 8*s+bob, 5.5*s, (13+la)*s);
        ctx.fillRect(5*s, 8*s+bob, 5.5*s, (13-la)*s);
        
        if (d.sz >= 28) {
            ctx.fillRect(-14*s, 8*s+bob, 5.5*s, (13-la)*s);
            ctx.fillRect(-24*s, 8*s+bob, 5.5*s, (13+la)*s);
        }
        
        if (['trex','carnotaurus','indoraptor','dilophosaurus','baryonyx','indominus','giganotosaurus'].includes(key)) {
            ctx.fillStyle = cLegs;
            ctx.fillRect(10*s, -2*s+bob, 3*s, 8*s);
            ctx.fillRect(12*s, 5*s+bob, 4*s, 3*s);
        }
        ...
        if (key === 'stegosaurus') {
            ...
            for (let i = -2; i <= 2; i++) {
                ctx.beginPath();
                ctx.moveTo(i*7*s, -5*s+bob);
                ctx.lineTo(i*7*s - 3*s, -20*s+bob);
                ctx.lineTo(i*7*s + 3*s, -20*s+bob);
                ctx.fill();
            }
            ctx.fillStyle = '#aa3030';
            ctx.fillRect(-28*s, 5*s+bob, 4*s, 4*s);
            ctx.fillRect(-32*s, 3*s+bob, 4*s, 4*s);
        }
        
        if (key === 'ankylosaurus') {
            ctx.fillStyle = '#aa8030';
            ctx.beginPath();
            ctx.ellipse(-45*s, 6*s+bob, 10*s, 8*s, 0, 0, Math.PI*2);
            ctx.fill();
        }
        
        if (key === 'triceratops') {
            ctx.fillStyle = '#ccc';
            ctx.fillRect(20*s, -14*s+bob, 3*s, 10*s);
            ctx.fillRect(15*s, -18*s+bob, 3*s, 14*s);
            ctx.fillRect(25*s, -10*s+bob, 3*s, 6*s);
        }
        
        if (key === 'spinosaurus') {
            ctx.fillStyle = '#7090cc';
            for (let i = -2; i <= 2; i++) {
                ctx.beginPath();
                ctx.moveTo(i*7*s, -5*s+bob);
                ctx.lineTo(i*7*s - 2*s, -28*s+bob);
                ctx.lineTo(i*7*s + 2*s, -28*s+bob);
                ctx.fill();
            }
        }
        
        if (key === 'dilophosaurus') {
            ctx.fillStyle = '#cc6020';
            ctx.beginPath();
            ctx.ellipse(18*s, -14*s+bob, 8*s, 4*s, 0, 0, Math.PI*2);
            ctx.fill();
        }
        
        if (key === 'parasaurolophus') {
            ctx.fillStyle = '#cc6030';
            ctx.beginPath();
            ctx.moveTo(15*s, -10*s+bob);
            ctx.quadraticCurveTo(5*s, -30*s+bob, -10*s, -20*s+bob);
            ctx.lineTo(-8*s, -16*s+bob);
            ctx.quadraticCurveTo(7*s, -22*s+bob, 16*s, -8*s+bob);
            ctx.fill();
        }
        ...
        if (key === 'indoraptor' || key === 'indominus') {
            if (key === 'indoraptor') {
                ...
                ctx.beginPath();
                ctx.ellipse(0, 0+bob, 20*s, 12*s, 0, 0, Math.PI*2);
                ctx.fill();
                
                ctx.fillStyle = '#ffff00';
                for (let i = 0; i < 3; i++) {
                    ctx.fillRect((-6 + i*6)*s, -6*s+bob, 2.5*s, 12*s);
                }
            } else {
                ctx.fillStyle = cLegs;
                for (let i = -2; i <= 3; i++) {
                    ctx.beginPath();
                    ctx.moveTo(i*8*s, -10*s+bob);
                    ctx.lineTo(i*8*s - 4*s, -22*s+bob);
                    ctx.lineTo(i*8*s + 4*s, -22*s+bob);
                    ctx.fill();
                }
            }
        ...
        if (key === 'dimetrodon') {
            ctx.fillStyle = cLegs;
            ctx.beginPath();
            ctx.moveTo(-15*s, -5*s+bob);
            ctx.quadraticCurveTo(0, -30*s+bob, 15*s, -5*s+bob);
            ctx.fill();
        }
        
        if (key === 'pachycephalosaurus') {
            ctx.fillStyle = '#e8c898';
            ctx.beginPath();
            ctx.ellipse(26*s, -13*s+bob, 6*s, 5*s, 0, 0, Math.PI*2);
            ctx.fill();
        }
        
        if (key === 'giganotosaurus') {
            ctx.fillStyle = cLegs;
            for (let i = -2; i <= 1; i++) {
                ctx.fillRect((i*8)*s, -18*s+bob, 4*s, 8*s);
            }
        }
        ...
        if (d.rarity === 'Legendary') {
            ...
            ctx.lineWidth = 2;
            ctx.beginPath();
            ctx.ellipse(0, 0+bob, 23*s, 15*s, 0, 0, Math.PI*2);
            ctx.stroke();
            ctx.shadowBlur = 0;
        }
        
        if (d.rarity === 'Epic') {
            ctx.shadowColor = RARITY_COLOR.Epic;
            ctx.shadowBlur = 18;
            ctx.strokeStyle = 'rgba(200,68,255,0.4)';
            ctx.lineWidth = 1.5;
            ctx.beginPath();
            ctx.ellipse(0, 0+bob, 23*s, 15*s, 0, 0, Math.PI*2);
            ctx.stroke();
        ...
        if (d.rarity === 'Boss' && key === 'indominus') {
            ...
            ctx.lineWidth = 3;
            ctx.beginPath();
            ctx.ellipse(0, 0+bob, 25*s, 16*s, 0, 0, Math.PI*2);
            ctx.stroke();
            ctx.shadowBlur = 0;
        }
    }
...
function drawHat(type, cx, cy, sc){
    ...
    if (!type || type === 'bucket') {
        ctx.save();
        ctx.translate(-11*s, -18*s);
        ctx.rotate(-0.35);
        const hg1 = ctx.createLinearGradient(0, -20*s, 0, 0);
        hg1.addColorStop(0, '#00ffee');
        hg1.addColorStop(1, '#009999');
        ctx.fillStyle = hg1;
        ctx.beginPath();
        ctx.moveTo(0,0);
        ctx.lineTo(-4*s, -22*s);
        ...
        ctx.translate(11*s, -18*s);
        ctx.rotate(0.35);
        const hg2 = ctx.createLinearGradient(0, -20*s, 0, 0);
        hg2.addColorStop(0, '#00ffee');
        hg2.addColorStop(1, '#009999');
        ctx.fillStyle = hg2;
        ctx.beginPath();
        ctx.moveTo(0,0);
        ctx.lineTo(-4*s, -22*s);
        ...
        const rimG = ctx.createLinearGradient(0, -22*s, 0, -16*s);
        rimG.addColorStop(0, '#4a5060');
        rimG.addColorStop(1, '#1e2228');
        
        ctx.fillStyle = rimG;
        ctx.beginPath();
        ctx.ellipse(0, -18*s, 20*s, 4.5*s, 0, 0, Math.PI*2);
        ctx.fill();
        ...
        ctx.beginPath();
        ctx.ellipse(0, 4*s, 14.5*s, 3.5*s, 0, 0, Math.PI*2);
        ctx.fill();
        ...
        for (let i = -1; i <= 1; i++) {
            ctx.beginPath();
            ctx.moveTo(i*7*s, -14*s);
            ctx.lineTo(i*6*s, 2*s);
            ctx.stroke();
        }
        ...
        ctx.beginPath();
        ctx.ellipse(0, -8*s, 16*s, 10*s, 0, 0, Math.PI*2);
        ctx.fill();
        
        ctx.fillStyle = '#e67300';
        const spikes = [[-10,-20,-18,-2], [-2,-24,-8,4], [6,-22,12,0], [12,-18,18,6]];
        spikes.forEach(sp => {
            ctx.beginPath();
            ctx.moveTo(sp[0]*s, sp[1]*s);
            ctx.quadraticCurveTo(sp[2]*s, sp[3]*s, (sp[2]-4)*s, (sp[3]+8)*s);
            ctx.lineTo(0, -5*s);
            ctx.fill();
        });
        
        ctx.fillStyle = '#994d00';
        const spikes2 = [[-14,-14,-22,5], [-6,-26,-14,-5], [2,-26,6,-5], [16,-12,22,8]];
        spikes2.forEach(sp => {
            ctx.beginPath();
            ctx.moveTo(sp[0]*s, sp[1]*s);
            ctx.quadraticCurveTo(sp[2]*s, sp[3]*s, (sp[2]+4)*s, (sp[3]+6)*s);
            ctx.lineTo(0, -5*s);
            ctx.fill();
        });
    } else if (type === 'dino_hood') {
        const hg = ctx.createLinearGradient(0, -20*s, 0, 5*s);
        hg.addColorStop(0, '#5cd65c');
        hg.addColorStop(1, '#2eb82e');
        ...
        ctx.beginPath();
        ctx.moveTo(-16*s, 8*s);
        ctx.quadraticCurveTo(-20*s, -18*s, 0, -22*s);
        ctx.quadraticCurveTo(20*s, -18*s, 16*s, 8*s);
        ctx.quadraticCurveTo(0, 14*s, -16*s, 8*s);
        ctx.fill();
        
        ctx.fillStyle = '#ffffe6';
        for (let i = -2; i <= 2; i++) {
            ctx.beginPath();
            ctx.moveTo((i*5 - 2)*s, 9*s);
            ctx.lineTo((i*5)*s, 15*s);
            ctx.lineTo((i*5 + 2)*s, 9*s);
            ctx.fill();
        }
        
        ctx.fillStyle = '#0a0a0a';
        ctx.beginPath();
        ctx.ellipse(-8*s, -6*s, 2.5*s, 3.5*s, -0.2, 0, Math.PI*2);
        ctx.fill();
        ...
        ctx.fillStyle = '#fff';
        ctx.beginPath();
        ctx.arc(-8.5*s, -7*s, 1*s, 0, Math.PI*2);
        ctx.fill();
        
        ctx.beginPath();
        ctx.arc(7.5*s, -7*s, 1*s, 0, Math.PI*2);
        ctx.fill();
    } else if (type === 'smiley') {
        const hg = ctx.createLinearGradient(0, -22*s, 0, 0);
        hg.addColorStop(0, '#ffe600');
        hg.addColorStop(1, '#e6b800');
        ...
        ctx.fillStyle = hg;
        ctx.beginPath();
        ctx.ellipse(0, 4*s, 16*s, 3*s, 0, 0, Math.PI*2);
        ctx.fill();
        
        ctx.fillStyle = '#ffdb4d';
        ctx.beginPath();
        ctx.ellipse(0, -20*s, 12*s, 3*s, 0, 0, Math.PI*2);
        ctx.fill();
        
        ctx.fillStyle = '#1a1a1a';
        ctx.beginPath();
        ctx.ellipse(-4*s, -10*s, 1.2*s, 2*s, 0, 0, Math.PI*2);
        ctx.fill();
        ...
        ctx.beginPath();
        ctx.moveTo(-5*s, -4*s);
        ctx.quadraticCurveTo(0, 1*s, 5*s, -4*s);
        ctx.stroke();
    }
...
function drawWorld(){
    ...
    for (let ty = Math.max(0, sy0); ty < Math.min(WS, sy1); ty++) {
        for (let tx = Math.max(0, sx0); tx < Math.min(WS, sx1); tx++) {
            ...
            if (t === 2) {
                const w = Math.sin(G.tick * 0.025 + tx * 0.6 + ty * 0.4) * 0.12 + 0.12;
                ctx.fillStyle = G.level === 2 ? `rgba(255,200,50,${w})` : `rgba(255,255,255,${w})`;
                ctx.fillRect(px + 4, py + TS/2 - 2, TS - 8, 4);
            }
            if (t === 1 && (tx * 7 + ty * 11) % 5 === 0) {
                ctx.fillStyle = G.level === 2 ? '#1a0a05' : '#1a4015';
                ctx.beginPath();
                ctx.arc(px + TS/2, py + TS/2, 11, 0, Math.PI*2);
                ctx.fill();
                
                ctx.fillStyle = G.level === 2 ? '#2a1005' : '#2d6028';
                ctx.beginPath();
                ctx.arc(px + TS/2, py + TS/2 - 5, 8, 0, Math.PI*2);
                ctx.fill();
            }
            if (t === 0 && (tx * 13 + ty * 7) % 9 === 0) {
                ctx.fillStyle = G.level === 2 ? '#2a1a12' : '#3a7a30';
                ctx.fillRect(px + 6, py + 22, 2.5, 8);
                ctx.fillRect(px + 13, py + 19, 2.5, 11);
                ctx.fillRect(px + 22, py + 24, 2.5, 6);
            }
            if (t === 3 && (tx * 3 + ty * 9) % 7 === 0) {
                ctx.strokeStyle = G.level === 2 ? 'rgba(120,50,20,0.5)' : 'rgba(180,150,60,0.4)';
                ctx.lineWidth = 1;
                ctx.beginPath();
                ctx.arc(px + TS/2, py + TS/2, 8, 0, Math.PI);
                ctx.stroke();
            }
            if (t === 4) {
                const pulse = Math.sin(G.tick * 0.07 + tx * 0.6 + ty * 0.4) * 0.25 + 0.75;
                const flow = Math.sin(G.tick * 0.04 + tx * 0.3 - ty * 0.2) * 6;
                
                ctx.fillStyle = '#2a0500';
                ctx.fillRect(px, py, TS, TS);
                
                ctx.fillStyle = 'rgba(200,40,0,' + (pulse * 0.9).toFixed(2) + ')';
                ctx.fillRect(px + 2, py + 2, TS - 4, TS - 4);
                
                ctx.fillStyle = 'rgba(255,120,0,' + (pulse * 0.85).toFixed(2) + ')';
                ctx.beginPath();
                ctx.ellipse(px + TS/2, py + TS/2 + flow, TS * 0.38, TS * 0.28, 0.3, 0, Math.PI*2);
                ctx.fill();
...
function drawHazards() {
    for (const h of G.hazards) {
        const sx = h.x - G.cam.x;
        const sy = h.y - G.cam.y;
        ...
        const pct = 1 - (h.life / h.maxLife);
        const rockY = sy - 600 * (1 - pct);
        
        ctx.fillStyle = '#4a2010';
        ctx.beginPath(); 
        ctx.arc(sx, rockY, 18, 0, Math.PI * 2); 
        ctx.fill();
        
        ctx.fillStyle = '#ff4400';
        ctx.beginPath(); 
        ctx.arc(sx, rockY, 12, 0, Math.PI * 2); 
        ctx.fill();
    }
}
...
function drawWilds(){
    for (const w of G.wilds) {
        const sx = w.x - G.cam.x;
        const sy = w.y - G.cam.y;
        ...
        ctx.fillRect(sx - 32, sy - dn.sz - 25, 64, 14);
        
        ctx.fillStyle = RARITY_COLOR[dn.rarity];
        ctx.font = 'bold 9px Courier New';
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.fillText(dn.name, sx, sy - dn.sz - 18);
    }
}
...
function drawPlayer(){
    ...
    const asz = 10;
    const ay = sy - DINOS[p.dk].sz - 32 + Math.sin(G.tick * 0.08) * 4;
    
    ctx.fillStyle = '#fff';
    ctx.beginPath();
    ctx.moveTo(sx, ay);
    ctx.lineTo(sx - asz, ay - asz);
    ctx.lineTo(sx + asz, ay - asz);
    ctx.fill();
    
    if (G.coop.partnerId) {
        ctx.fillStyle = 'rgba(40,20,100,0.7)'; 
        ctx.fillRect(sx - 60, sy - DINOS[p.dk].sz - 50, 120, 16);
    ...
    const pct = G.playerHp / pMaxHp();
    const col = pct > 0.5 ? '#44cc44' : pct > 0.25 ? '#cccc44' : '#cc4444';
    
    hpBar(sx - 28, sy - DINOS[p.dk].sz - 18, 56, 8, G.playerHp, pMaxHp(), col);
    
    if (G.playerShield > 0) {
        hpBar(sx - 28, sy - DINOS[p.dk].sz - 26, 56, 6, G.playerShield, Math.floor(pMaxHp()*0.4), '#44aaff');
    }
}
...
function drawHUD(){
    ...
    ctx.textAlign = 'left';
    ctx.textBaseline = 'middle';
    ctx.fillText(`🪣 ${G.wheat}`, 16, 18);
    ctx.shadowBlur = 0;
    
    ctx.fillStyle = '#aaa'; 
    ctx.font = '12px Courier New';
    
    let mapStr = G.level === 1 ? 'Isla Uno' : 'Volcano Island';
    if(G.room === 'cave') mapStr += ' [Cave]';
    if(G.room === 'mountain') mapStr += ' [Mountain]';
    ctx.fillText(`Map: ${mapStr}`, 16, 38);
    ...
    const d = pDino();
    ctx.fillStyle = RARITY_COLOR[d.rarity];
    ctx.shadowColor = RARITY_COLOR[d.rarity];
    ctx.shadowBlur = 10;
    ctx.font = 'bold 15px Courier New';
    ctx.textAlign = 'center';
    ctx.fillText(`${d.em} ${d.name}[${d.rarity}]`, W / 2, G.cheatsActive ? 18 : 26);
    ctx.shadowBlur = 0;

    const pct = G.playerHp / pMaxHp();
    const hcol = pct > 0.5 ? '#44cc44' : pct > 0.25 ? '#cccc44' : '#cc4444';
    ...
    if (!isConnected) {
        ...
        btn(W - 55, 56, 40, 20, 'Join', '#cc2288', '#fff', () => showMpModal('join'));
    } else {
    ...
    if (G.coop.partnerId) {
        btn(16, 56, 130, 20, 'Leave Party', '#cc2244', '#fff', () => {
            if (confirm("Are you sure you want to leave the party? You will return to your solo save!")) {
                sendCoop({ type: 'coop_break', target: G.coop.partnerId });
    ...
    btn(bsx, by, bw, bh, 'Index', '#2255cc', '#fff', () => {
        G.state = 'index';
        G.idxPage = 0;
        G.btns = [];
    }, '📘');
    
    btn(bsx + bw + gap, by, bw, bh, 'Shop', '#cc5522', '#fff', () => {
        G.state = 'shop';
        G.btns = [];
    }, '🛒');

    btn(bsx + 2*(bw + gap), by, bw, bh, 'Shld(40)', '#225588', '#fff', () => {
        const maxS = Math.floor(pMaxHp() * 0.4);
        const addS = Math.floor(pMaxHp() * 0.15);
        if (G.wheat >= 40 && G.playerShield < maxS) {
            G.wheat -= 40;
            G.playerShield = Math.min(maxS, G.playerShield + addS);
            spawnParticles(W / 2, H - 80, '#44aaff', 12);
            if (G.coop.partnerId) sendCoop({ type: 'coop_wheat', target: G.coop.partnerId, wheat: G.wheat });
        }
    }, '🛡️');

    btn(bsx + 3*(bw + gap), by, bw, bh, 'Heal(5)', '#225522', '#fff', () => {
        const addH = Math.floor(pMaxHp() * 0.1);
        if (G.wheat >= 5 && G.playerHp < pMaxHp()) {
            G.wheat -= 5;
            G.playerHp = Math.min(pMaxHp(), G.playerHp + addH);
            spawnParticles(W / 2, H - 80, '#44ff44', 8);
            if (G.coop.partnerId) sendCoop({ type: 'coop_wheat', target: G.coop.partnerId, wheat: G.wheat });
        }
    }, '💊');
    ...
    for (let id in G.otherPlayers) {
        ...
        ctx.fillStyle = '#aaddff';
        ctx.beginPath();
        ctx.arc(mmx + op.x/TS*msc, mmy + op.y/TS*msc, 2.5, 0, Math.PI*2);
        ctx.fill();
    }
    ...
    for (const w of G.wilds) {
        ...
        ctx.beginPath();
        ctx.arc(mmx + w.x/TS*msc, mmy + w.y/TS*msc, w.isBoss ? 3 : 1.5, 0, Math.PI*2);
        ctx.fill();
    }
    ...
    if (G.coop.reqFrom) {
        ...
        ctx.font = 'bold 15px Courier New';
        ctx.textAlign = 'center';
        ctx.fillText(G.coop.reqFromName + ' sent a Co-op request!', rx + rw/2, ry + 25);
        ...
        ctx.font = 'bold 15px Courier New';
        ctx.textAlign = 'center';
        ctx.fillText(G.pvp.reqFromName + ' requested PvP!', W/2, ry + 25);
        ...
        btn(rx + 150, ry + 45, 110, 26, 'Accept', '#22cc22', '#fff', () => {
            sendPvP({ type: 'pvp_reply', target: G.pvp.reqFrom, sender: (G.isHost ? 'host' : G.peer.id), accept: true, stats: getMyBattleStats() });
    ...
    if (G.pvp.msgTimer > 0) {
        ...
        ctx.textAlign = 'center';
        ctx.fillText("PvP request is on cooldown!", W/2, H/2 - 60 - (180 - G.pvp.msgTimer) * 0.2);
        ctx.shadowBlur = 0;
    }

    btn(16, 280, 65, 40, 'Chat', '#44aa44', '#fff', () => {
...
function drawBattle(){
    ...
    bg.addColorStop(0, boss ? '#1a0303' : '#060e1a');
    bg.addColorStop(1, boss ? '#2a0808' : '#080f1f');
    ...
    ctx.beginPath();
    ctx.ellipse(W/2, H*0.62, W*0.42, H*0.13, 0, 0, Math.PI*2);
    ctx.fill();
    ...
    if (boss) {
        ...
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.fillText('⚠ BOSS BATTLE ⚠', W/2, 25);
        ctx.shadowBlur = 0;
    }
    ...
    ctx.fillStyle = RARITY_COLOR[en.rarity];
    ...
    if (b.isCoop) {
        ...
        if (b.php_shield > 0) {
            hpBar(W*0.02, H*0.5 + 16, W*0.3, 8, b.php_shield, b.pmhp_shield, '#44aaff');
        }
        ...
        if (b.cpShield > 0) {
            hpBar(W*0.02, H*0.75 + 16, W*0.3, 8, b.cpShield, Math.floor(b.cpMhp * 0.4), '#44aaff');
        }
        ctx.fillStyle = '#aaddff';
        ctx.font = 'bold 11px Courier New';
        ctx.textAlign = 'left';
    ...
    for(const dn of b.dnums){
        ...
        ctx.font = `bold ${Math.max(10, 26 - dn.life * 0.1)}px Courier New`;
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.fillText(`-${dn.val}`, dn.x, dn.y - (60 - dn.life) * 0.9);
    }
    ctx.globalAlpha = 1;
    ...
    if (b.res === 'level2') {
        ...
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.fillText('🏆 MEGALODON DEFEATED!', W/2, H/2 - 35);
        ctx.shadowBlur = 0;
        ...
        const bx = W*0.65;
        const by = b.isCoop ? H*0.83 : H*0.73;
        ...
        ctx.shadowColor = win ? '#44ff44' : '#ff4444';
        ctx.shadowBlur = 15;
        ctx.font = 'bold 30px Courier New';
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.fillText(win ? '🏆 VICTORY!' : '💀 DEFEATED!', W/2, H/2 - 35);
        ctx.shadowBlur = 0;
        
        ctx.fillStyle = '#ddd';
        ctx.font = '13px Courier New';
        ctx.fillText(b.log[0] || '', W/2, H/2);
        btn(W/2 - 70, H/2 + 25, 140, 40, 'Continue', '#224488', '#fff', exitBattle, '▶');
    }
}
...
function drawIndex(){
    ...
    for (let i = 0; i < 60; i++) {
        ...
        ctx.beginPath();
        ctx.arc((i*137 + i*i)%W, (i*89 + i*31)%H, 0.8, 0, Math.PI*2);
        ctx.fill();
    }
    
    ctx.fillStyle = 'rgba(10,25,60,0.95)';
    ctx.fillRect(0, 0, W, 52);
    ...
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText(G.level === 1 ? '📘 DINO INDEX (MAP 1)' : '📘 DINO INDEX (MAP 2)', W/2, 26);
    ctx.shadowBlur = 0;
    ...
    ctx.textAlign = 'left';
    ctx.textBaseline = 'middle';
    ctx.fillText(`${ak.filter(k => G.discovered[k]).length}/${ak.length} Discovered`, 20, 26);
    
    const cw = Math.min(145, Math.floor((W - 30) / Math.max(1, Math.floor((W - 30) / 145))));
    const cols = Math.max(1, Math.floor((W - 30) / cw));
    const rows = Math.max(1, Math.floor((H - 90) / 130));
    const per = cols * rows;
    const pg = G.idxPage;
    const si = pg * per;
    const ei = Math.min(si + per, ak.length);
    
    for (let i = si; i < ei; i++) {
        const key = ak[i];
        const dn = DINOS[key];
        const col = (i - si) % cols;
        const row = Math.floor((i - si) / cols);
        const cx = 15 + col*cw + cw/2;
        const cy = 68 + row*130 + 62;
        const found = G.discovered[key];
        const isCur = G.player.dk === key;
        ...
        if (found) {
            ctx.save();
            ctx.beginPath();
            ctx.rect(cx - cw/2 + 6, cy - 55, cw - 12, 80);
            ctx.clip();
            const idxOc = isCur ? G.player.oc : null;
            drawDino(key, cx, cy - 18, 1, G.tick * 0.5, 0.65, 1, idxOc);
            ctx.restore();
        } else {
        ...
        ctx.fillStyle = found ? RARITY_COLOR[dn.rarity] : '#444';
        ctx.font = 'bold 10px Courier New';
        ctx.textAlign = 'center';
        ctx.textBaseline = 'bottom';
        ctx.fillText(found ? dn.name : '???', cx, cy + 44);
        
        if (found) {
            ctx.fillStyle = 'rgba(0,0,0,0.6)';
            ctx.fillRect(cx - 28, cy + 45, 56, 13);
            ctx.fillStyle = RARITY_COLOR[dn.rarity];
            ctx.font = '8px Courier New';
            ctx.fillText(dn.rarity, cx, cy + 58);
            
            if (!isCur) {
                btn(cx - 32, cy + 61, 64, 22, 'Equip', '#1a3a88', '#fff', () => {
    ...
    if (tp > 1) {
        if(pg > 0) btn(14, H - 48, 80, 34, '◀ Prev', '#1a2a44', '#fff', () => G.idxPage--);
        ctx.fillStyle = '#777';
        ctx.font = '12px Courier New';
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.fillText(`${pg + 1}/${tp}`, W/2, H - 30);
        if(pg < tp - 1) btn(W - 94, H - 48, 80, 34, 'Next ▶', '#1a2a44', '#fff', () => G.idxPage++);
    }
    
    btn(W - 86, 8, 78, 34, '✕ Close', '#4a2010', '#fff', () => {
        G.state = 'world';
        G.btns = [];
    });
}
...
function drawShop(){
    ...
    for (let i = 0; i < W; i += 60) {
        ctx.strokeStyle = 'rgba(200,150,0,0.03)';
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.moveTo(i, 0);
        ctx.lineTo(i, H);
    ...
    ctx.fillStyle = 'rgba(60,30,0,0.95)';
    ctx.fillRect(0, 0, W, 52);
    ...
    ctx.textAlign = 'left';
    ctx.textBaseline = 'middle';
    ctx.fillText(`🪣 ${G.wheat}`, 16, 26);

    const upgs = [
        {k:'hp', label:'HP Boost', desc:'+28 Max HP', icon:'❤️', base:25, col:'#aa2222', max:35},
    ...
    upgs.forEach((upg, i) => {
        ...
        ctx.textAlign = 'center';
        ctx.textBaseline = 'top';
        ctx.fillText(upg.icon, x + cw2/2, y + 14);
        
        ctx.fillStyle = '#fff';
        ctx.font = 'bold 15px Courier New';
        ctx.textBaseline = 'top';
        ctx.fillText(upg.label, x + cw2/2, y + 62);
        
        ctx.fillStyle = maxed ? '#ffaa00' : '#ffd700';
        ctx.font = 'bold 13px Courier New';
        ctx.fillText(maxed ? `Lv ${lv} ✓` : `Lv ${lv}`, x + cw2/2, y + 83);
        
        ctx.fillStyle = '#aaa';
        ctx.font = '11px Courier New';
        ctx.fillText(upg.desc, x + cw2/2, y + 104);
        
        if (maxed) {
            ctx.fillStyle = '#ffaa00';
            ctx.font = 'bold 13px Courier New';
            ctx.fillText('MAX UPGRADE', x + cw2/2, y + 126);
        } else {
            ctx.fillStyle = can ? '#aaddff' : '#664422';
            ctx.font = 'bold 13px Courier New';
            ctx.fillText(`Cost: 🪣 ${cost}`, x + cw2/2, y + 126);
        }
        
        btn(x + 16, y + 154, cw2 - 32, 42, maxed ? 'MAX UPGRADE' : 'UPGRADE',
        maxed ? '#553300' : can ? upg.col : '#2a2020',
        maxed ? '#ffaa00' : can ? '#fff' : '#665', () => {
            if (!maxed && G.wheat >= cost) {
                G.wheat -= cost;
                G.player.upg[upg.k]++;
                if (upg.k === 'hp') {
                    G.playerHp += Math.floor(28 * R_MULT[pDino().rarity]);
                }
        ...
        ctx.fillStyle = '#111';
        ctx.fillRect(x + 16, y + ch - 20, cw2 - 32, 10);
        
        ctx.fillStyle = upg.col;
        ctx.fillRect(x + 16, y + ch - 20, Math.max(0, (cw2 - 32) * (lv / maxLv)), 10);
    });

    const hy = 65 + ch + 20;
    const hw = Math.min(420, W - 60);
    const hx = (W - hw) / 2;
    const hhw = (hw - 10) / 2;
    ...
    ctx.fillText('🛡️', hx + hhw/2, hy + 10);
    ctx.fillStyle = '#aaddff';
    ctx.font = 'bold 14px Courier New';
    ctx.fillText('+15% Shield', hx + hhw/2, hy + 42);
    ctx.fillText(`Cost: 🪣 ${scost}`, hx + hhw/2, hy + 60);
    
    btn(hx, hy, hhw, 80, '', scan ? 'rgba(0,0,0,0.01)' : 'rgba(0,0,0,0.01)', 'rgba(0,0,0,0)', () => {
        const maxS = Math.floor(pMaxHp() * 0.4);
        const addS = Math.floor(pMaxHp() * 0.15);
        if (G.wheat >= scost && G.playerShield < maxS) {
            G.wheat -= scost;
            G.playerShield = Math.min(maxS, G.playerShield + addS);
            spawnParticles(W/2, H/2, '#44aaff', 14);
            if (G.coop.partnerId) sendCoop({ type: 'coop_wheat', target: G.coop.partnerId, wheat: G.wheat });
        }
    });
    ...
    ctx.font = '24px sans-serif';
    ctx.fillText('💊', hx + hhw + 10 + hhw/2, hy + 10);
    ctx.fillStyle = '#aaffaa';
    ctx.font = 'bold 14px Courier New';
    ctx.fillText('+10% Heal', hx + hhw + 10 + hhw/2, hy + 42);
    ctx.fillText(`Cost: 🪣 ${hcost}`, hx + hhw + 10 + hhw/2, hy + 60);
    
    btn(hx + hhw + 10, hy, hhw, 80, '', hcan ? 'rgba(0,0,0,0.01)' : 'rgba(0,0,0,0.01)', 'rgba(0,0,0,0)', () => {
        if (G.wheat >= hcost && G.playerHp < pMaxHp()) {
            G.wheat -= hcost;
            G.playerHp = Math.min(pMaxHp(), G.playerHp + Math.floor(pMaxHp() * 0.1));
            spawnParticles(W/2, H/2, '#44ff44', 14);
            if (G.coop.partnerId) sendCoop({ type: 'coop_wheat', target: G.coop.partnerId, wheat: G.wheat });
        }
    });
...
function drawCustomize(){
    ...
    for (let i = 0; i < W; i += 60) {
        ctx.strokeStyle = 'rgba(150,50,255,0.03)';
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.moveTo(i, 0);
        ctx.lineTo(i, H);
    ...
    ctx.fillStyle = 'rgba(40,10,80,0.95)';
    ctx.fillRect(0, 0, W, 52);
    ...
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText('🧢 AVATAR CUSTOMIZATION', W/2, 26);
    ctx.shadowBlur = 0;
    ...
    hats.forEach((h, i) => {
        ...
        if (!isEq) {
            btn(x + 20, y + 80, cw - 40, 15, 'Equip', '#5522aa', '#fff', () => {
                G.player.hat = h.id;
            });
        } else {
            ctx.fillStyle = '#44ff44';
    ...
    CUST_COLS.forEach((col, i) => {
        ...
        ctx.beginPath();
        ctx.arc(rx + 25, ry + 20, 18, 0, Math.PI*2);
        ctx.fill();
...
function drawIntro(){
    ...
    const mp = G.tick % 300;
    if (mp < 60) {
        ctx.strokeStyle = `rgba(255,200,100,${(60 - mp)/60 * 0.8})`;
        ctx.lineWidth = 2;
        ctx.beginPath();
        ctx.moveTo(mp * 8, mp * 2);
        ctx.lineTo(mp * 8 - 30, mp * 2 - 10);
        ctx.stroke();
    }
    
    ctx.fillStyle = 'rgba(0,80,20,0.12)';
    ctx.beginPath();
    ctx.ellipse(W/2, H*0.35, 320, 80, 0, 0, Math.PI*2);
    ctx.fill();
    
    ctx.fillStyle = '#7ab648';
    ctx.shadowColor = '#3a7a18';
    ctx.shadowBlur = 60;
    ctx.font = `bold ${Math.min(62, W*0.065)}px Courier New`;
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText('🦖 DinoWorld', W/2, H*0.3);
    ctx.shadowBlur = 0;
    
    ctx.fillStyle = 'rgba(150,200,120,0.85)';
    ctx.font = `${Math.min(18, W*0.018)}px Courier New`;
    ...
    feats.forEach((f, i) => {
        const fx = W/2 + (i - 2) * Math.min(158, W/6.2);
        const fy = H * 0.5;
    ...
    if (window.activeSave) {
        ...
        if(Math.floor(G.tick / 28) % 2){
            ...
            ctx.fillText('[ Click or Press Any Key to Start ]', W/2, Math.max(iby + 75, H * 0.9));
            ctx.shadowBlur = 0;
        }
    }
}
...
function render(){
    ...
    if (G.state === 'world') {
        ...
        for (let id in G.otherPlayers) {
            ...
            if (sx > -100 && sx < canvas.width + 100 && sy > -100 && sy < canvas.height + 100) {
                ...
                // Render "Bonded" tag over your Co-op Partner
                if (op.coopPartner) {
                    ctx.fillStyle = '#dd88ff';
                    ctx.font = 'bold 10px Courier New';
                    ctx.textAlign = 'center';
                ...
                // Overworld HP Bar for Partner
                const opHp = op.hp || 1; 
                const opMhp = op.mhp || 1; 
                const hpPct = Math.max(0, opHp / opMhp); 
                const hpC = hpPct > 0.5 ? '#44cc44' : hpPct > 0.25 ? '#cccc44' : '#cc4444';
                
                ctx.fillStyle = 'rgba(0,0,0,0.8)'; 
                ctx.fillRect(sx - 20, sy - DINOS[op.dk].sz - 16, 40, 5); 
                ctx.fillStyle = hpC; 
                ctx.fillRect(sx - 20, sy - DINOS[op.dk].sz - 16, 40 * hpPct, 5); 
                ctx.strokeStyle = 'rgba(255,255,255,0.3)'; 
                ctx.lineWidth = 1; 
                ctx.strokeRect(sx - 20, sy - DINOS[op.dk].sz - 16, 40, 5);
            }
        }
        ...
        if (G.joy.on) {
            ...
            ctx.beginPath();
            ctx.arc(jx + Math.cos(ja)*jl, jy + Math.sin(ja)*jl, 26, 0, Math.PI*2);
            ctx.fill();
        }
    ...
    // NEW: DRAW THE BLACK FADE OVERLAY!
    if (G.fade.active) {
