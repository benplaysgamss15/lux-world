// ── SAVE & LOAD SYSTEM (CLOUD EDITION) ──
window.activeSave = null;

// This tells the game to check the cloud when it starts
...
function saveGame() {
    // PROTECT SOLO DATA: Do not overwrite if in a Co-op Party!
    if (G.state === 'intro' || G.coop.partnerId) return; 
    
    // Call the Firebase function we built in the other file
    if (window.cloudSave) {
        window.cloudSave(G);
    }
}

// Save when the tab is closed
